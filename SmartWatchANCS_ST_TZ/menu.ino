...
static const char PROGMEM mainMenuStrings0[] = "Arcade";
static const char PROGMEM mainMenuStrings1[] = "Shortcuts";
static const char PROGMEM mainMenuStrings2[] = "Settings";
...
void settingsMenu(uint8_t selection) {
  if (menu_debug_print)SerialMonitorInterface.println("settingsMenuHandler");
  if (selection == 0) {  // datetime
    newMenu(dateTimeMenuIndex);
  }
  if (selection == 1) { // update sleep timeout
    char buffer[20];
    strcpy_P(buffer, (PGM_P)pgm_read_word(&(menuList[settingsMenuIndex].strings[selection])));
    editInt(0, &sleepTimeout, buffer, NULL);
  }
  if (selection == 2) {  // update brightness
    char buffer[20];
    strcpy_P(buffer, (PGM_P)pgm_read_word(&(menuList[settingsMenuIndex].strings[selection])));
...
void playSong() {
  ...
  Keyboard.press(0x72); // hold "r"
  Keyboard.releaseAll();  // release both win and r key together
  ...
  Keyboard.write(KEY_RETURN);
  delay(2000);
  Keyboard.write(0x32); // press spacebar to play song
}
