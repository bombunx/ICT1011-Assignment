...
void displayBattery() {
  ...
  display.drawLine(x - 1, y, x - 1, y + height, 0xFFFF); //left boarder
  display.drawLine(x - 1, y - 1, x + length, y - 1, 0xFFFF); //top border
  display.drawLine(x - 1, y + height + 1, x + length, y + height + 1, 0xFFFF); //bottom border
  display.drawLine(x + length, y - 1, x + length, y + height + 1, 0xFFFF); //right border
  display.drawLine(x + length + 1, y + 2, x + length + 1, y + height - 2, 0xFFFF); //right border
  for (uint8_t i = 0; i < length; i++) {
